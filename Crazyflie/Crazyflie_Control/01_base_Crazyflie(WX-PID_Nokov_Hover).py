...
plotter = UAVPlotter()
logger_dir = r'E:\test\Crazyflie\Crazyflie_Control\Simulation_Data\GroundEffect_Nokov\Hover\RecordData'
...
# 负责通信、参数接管、日志记录以及调用外部 WX-PID
    ...
    # 外部 WX-PID 核心控制器与状态机
        ...
        if self.i < start_step_len:
            ...
            # x_offset = state_information[data_len - end_step_len - 1, 0] - state_information[0, 0]
            # y_offset = -(state_information[data_len - end_step_len - 1, 1] - state_information[0, 1])
            x_offset = 0.0
            y_offset = 0.0
            z = 0.04
            mode = 'land'
        ...
        # self.set_point.x = x_offset
        # self.set_point.y = y_offset
        self.set_point.z = z
...
if __name__ == '__main__':
    ...
    print(f"项目名称: Crazyflie——{os.path.basename(__file__)}")
    ...
    filename_record = f'Crazyflie_(PID)_data_z{int(z_func * 1000)}Hover{int(xhr)}.csv'
