# -*- coding: utf-8 -*-
"""
Created on Mon Jun 22 15:47:25 2026
@author: XHR
"""
# =============================== 库导入 ===============================
# ---------- 常规库导入 ----------
...
plotter = UAVPlotter()
logger_dir = r'E:\test\Crazyflie\Crazyflie_Control\Simulation_Data\GroundEffect_Nokov\SameV\RecordData'
...
dt = 1 / Control_freq
hover_step_len = 500
high_hover_step_len = 500
cycle_num = 4
end_step_len = 200
rise_step_len = int(round((high_z - low_z) / vertical_speed * Control_freq))
fall_step_len = rise_step_len

one_cycle_step_len = hover_step_len + rise_step_len + high_hover_step_len + fall_step_len
step = cycle_num * one_cycle_step_len + end_step_len
...
# 负责通信、参数接管、日志记录以及调用外部 WX-PID
    ...
    # 外部 WX-PID 核心控制器与状态机
        ...
        cycle_step = self.i % one_cycle_step_len
        cycle_id = self.i // one_cycle_step_len
        ...
        if cycle_id < cycle_num:
            if cycle_step < hover_step_len:
                ...
                # 2. 以 vertical_speed 从 low_z 匀速上升到 high_z
                ...
                # 4. 以 vertical_speed 从 high_z 匀速下降到 low_z
                fall_progress = (
                                        cycle_step - hover_step_len - rise_step_len - high_hover_step_len
                                ) / max(fall_step_len - 1, 1)
...
if __name__ == '__main__':
    ...
    filename_record = f'Crazyflie_(PID)_data_v{int(vertical_speed * 1000)}mmsSameVDifferentZ{int(xhr)}.csv'
